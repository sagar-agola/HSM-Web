...
export class FrequencyComponent implements OnInit {
    ...
    displayedColumns = ['id', 'frequencyName', 'edit', 'delete'];

    ELEMENT_DATA: IFrequency[] = [];
    jsonData: any[]=[];
    ...
    getDataSource(): void{
        this._frequencyService.getAllFrequencyByCustomerSites(this.customerId, this.siteId)
          .subscribe(res => {
            this.default = false;
            this.loading = false;
            this.ELEMENT_DATA = res;
            this.jsonData = res;
            this.dataSource = new MatTableDataSource<IFrequency>(this.ELEMENT_DATA);
            this.dataSource.paginator = this.paginator;
            this.dataSource.sort = this.sort;
          })
      }

    openDialogFrequency(opt: string, data: any) {
        if(this.canAccess('AddEditFrequency')) {
            ...
            const dialogRef = this.dialog.open(FrequencyModalComponent, dialogConfig);
    ...
    deleteAction(id: number, name: string): void {
        if(this.canAccess('DeleteFrequency')) {
            if (confirm("Are you sure to delete " + name + " ? This cannot be undone.")) {
                this._frequencyService.deleteFrequency(id)
                .subscribe(res => {    
    ...
    exportExcel(): void {
        if(this.canAccess('ExportFrequency')) {
