...
@Component({
    selector: "asset-businesstype",
    templateUrl: './assetbusinesstype.component.html',
    styleUrls: [
        './assetbusinesstype.component.scss'
    ]
})

export class AssetHierarchyBusinessTypeComponent implements OnInit {

    @ViewChild(MatPaginator, {static: true}) paginator: MatPaginator;
    @ViewChild(MatSort, {static: true}) sort: MatSort;
    @ViewChild('TABLE', {static: true}) table: ElementRef;

    displayedColumns = ['id', 'businessType', 'edit', 'delete'];

    ELEMENT_DATA: IAssetHierarchyBusinessTypeTaxonomy[] = [];
    jsonData: any[]=[];
    ...
    getDataSource(): void{
        this._assetBusinessTypeService.getAllBusinessTypeByCustomerSites(this.customerId, this.siteId)
          .subscribe(res => {
            this.default = false;
            this.loading = false;
            this.ELEMENT_DATA = res;
            this.jsonData = res;
            this.dataSource = new MatTableDataSource<IAssetHierarchyBusinessTypeTaxonomy>(this.ELEMENT_DATA);
            this.dataSource.paginator = this.paginator;
            this.dataSource.sort = this.sort;
          })
      }

    openDialogBusinessType(opt: string, data: any) {
        if(this.canAccess('AddEditAssetBusinessType')) {
            ...
            const dialogRef = this.dialog.open(AssetBusinessTypeModalComponent, dialogConfig);
    ...
    deleteAction(id: number, name: string): void {
        if(this.canAccess('DeleteAssetBusinessType')) {
            if (confirm("Are you sure to delete " + name + " ? This cannot be undone.")) {
                this._assetBusinessTypeService.deleteAssetBusinessType(id)
                .subscribe(res => {    
