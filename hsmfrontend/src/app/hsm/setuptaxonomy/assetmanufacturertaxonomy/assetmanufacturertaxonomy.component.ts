...
import { AssetHierarchyManufacturerModalComponent } from '../../modal/assetmanufacturermodal/assetmanufacturermodal.component';
import * as XLSX from 'xlsx';
import { PermissionManagerService } from 'src/app/shared/services/permission.manager.service';

@Component({
    selector: "asset-manufacturer",
    templateUrl: './assetmanufacturertaxonomy.component.html',
    styleUrls: [
        './assetmanufacturertaxonomy.component.scss'
    ]
})

export class AssetHierarchyManufacturerTaxonomyComponent implements OnInit {

    @ViewChild(MatPaginator, {static: true}) paginator: MatPaginator;
    @ViewChild(MatSort, {static: true}) sort: MatSort;
    @ViewChild('TABLE', {static: true}) table: ElementRef;

    displayedColumns = ['id', 'assetManufacturer', 'edit', 'delete'];

    ELEMENT_DATA: IAssetHierarchyManufacturerTaxonomy[] = [];
    jsonData: any[]=[];
    ...
    dataSource;
    fileName = 'AssetManufacturer.xlsx';
    ...
    getDataSource(): void{
        this._assetHierarchyManufacturerService.getAllManufacturerByCustomerSites(this.customerId, this.siteId)
          .subscribe(res => {
            this.default = false;
            this.loading = false;
            this.ELEMENT_DATA = res;
            this.jsonData = res;
            this.dataSource = new MatTableDataSource<IAssetHierarchyManufacturerTaxonomy>(this.ELEMENT_DATA);
    ...
    openDialogManufacturer(opt: string, data: any) {
        if(this.canAccess('AddEditAssetManufacturer')) {
            ...
            const dialogRef = this.dialog.open(AssetHierarchyManufacturerModalComponent, dialogConfig);
    ...
    deleteAction(id: number, name: string): void {
        if(this.canAccess('DeleteAssetManufacturer')) {
            if (confirm("Are you sure to delete " + name + " ? This cannot be undone.")) {
                this._assetHierarchyManufacturerService.deleteAssetManufacturer(id)
                .subscribe(res => {    
