...
import { SiteTgsViewModalComponent } from '../modal/sitetgsviewmodal/sitetgsviewmodal.component';
...
@Component({
  selector: "asset-task-group",
  templateUrl: './assettaskgroupstrategy.component.html',
  styleUrls: [
    './assettaskgroupstrategy.component.scss'
  ]
})

export class AssetTaskGroupStrategyComponent implements OnInit {
...
assetTaskGroupStrategyObject: IAssetTaskGroupStrategyHsm = {
  ...
  constructor(
    ...
    private _assignTGSMaterialService: AssignAssetTaskGroupStrategyMaterialSiteService,
  ...
  ngOnInit(): void {
    ...
    forkJoin(
    ...
    ).subscribe(([tt, fr, ty, om, ai, bt, at, pf, ac, as, am, cm, cb, sl]) => {
      ...
      this.customerList = cm.sort((a, b) => (a.customerName < b.customerName ? -1 : 1));
      this.siteList = sl.sort((a, b) => (a.siteName < b.siteName ? -1 : 1));
    ...
    this._assetTaskGroupStrategyService.getSiteTaskGroupStrategyHierarchy(this.customerId, this.siteId)
      .subscribe(res => {
        this.dataSource.data = JSON.parse(res['hierarchy']);
        // console.log(this.dataSource.data)
      })

  this._assetTaskGroupStrategyService.getDropdownTGSSiteAssetBusiness(this.categoryid)
      .subscribe(i => {
          this.businessTypeList = i.sort((a, b) => (a.dropName < b.dropName ? -1 : 1));
      });

  this._assetTaskGroupStrategyService.getDropdownTGSSiteAssetIndustry(this.categoryid)
      .subscribe(i => {
          this.industryList = i.sort((a, b) => (a.dropName < b.dropName ? -1 : 1));
      });
  this._assetTaskGroupStrategyService.getDropdownTGSSiteAssetProcessFunction(this.categoryid)
      .subscribe(s => {
          this.processFunctionList = s.sort((a, b) => (a.dropName < b.dropName ? -1 : 1));
      });
  this._assetTaskGroupStrategyService.getDropdownTGSSiteAssetSpec(this.categoryid)
      .subscribe(b => {
          this.specTaxonomyList = b.sort((a, b) => (a.dropName < b.dropName ? -1 : 1));
      });
  this._assetTaskGroupStrategyService.getDropdownTGSSiteAssetManufacturer(this.categoryid)
      .subscribe(m => {
          this.manufacturerTaxonomyList = m.sort((a, b) => (a.dropName < b.dropName ? -1 : 1));
      });
  this._assetTaskGroupStrategyService.getDropdownTGSSiteTaskType(this.categoryid)
      .subscribe(t => {
          this.taskTypeList = t.sort((a, b) => (a.dropName < b.dropName ? -1 : 1));
      });
  this._assetTaskGroupStrategyService.getDropdownTGSSiteTradeType(this.categoryid)
      .subscribe(ty => {
          this.tradeTypeList = ty.sort((a, b) => (a.dropName < b.dropName ? -1 : 1));
      });
  this._assetTaskGroupStrategyService.getDropdownTGSSiteFrequency(this.categoryid)
      .subscribe(fq => {
          this.frequencyList = fq.sort((a, b) => (a.dropName < b.dropName ? -1 : 1));
      });
  this._assetTaskGroupStrategyService.getDropdownTGSSiteOperationalMode(this.categoryid)
      .subscribe(d => {
  ...
  getDataSource(): void {
    this._assetTaskGroupStrategyService.getAssetTaskGroupStrategy(this.customerId, this.siteId)
      .subscribe(res => {
        // console.log(res);
        this.default = false;
        this.loading = false;
...
nodeClick(node: any){
  ...
  this._assetTaskGroupStrategyService.getAssetTaskGroupStrategyFilters(this.categoryid, this.category, this.taskGroupId, this.taskGroupDesc, this.taskTypeId, 
    this.frequencyId, this.tradeTypeId, this.operationalModeId, this.industryId, this.businessTypeId, 
    this.assetClassTaxonomyId, this.processFunctionId, this.assetSpecId, this.manufactureId, this.customerId, this.siteId)
      .subscribe(res => {
        this.default = false;
        this.loading = false;
        this.ELEMENT_DATA = res;
...
siteOnSelect(event){
  ...
  routeCreate(){
    if(this.canAccess('CreateSiteTGS')) {
      this.router.navigate(["/main/site-task-group-form"]);
    }else{
  ...
  onKeyupTask(name: string){
    if(name !== '')
    {
        this._assetTaskGroupStrategyService.getDropdownTGSSiteIdno(this.categoryid, name)
        .subscribe(res => {
            // console.log(res)
            this.taskIdList = res;
  ...
  statusOnSelect(event, id: number){
    ...
    this._assetTaskGroupStrategyService.getAssetTaskGroupStrategyById(id)
      .subscribe(res => {
          // console.log(res);
          ...
          this._assetTaskGroupStrategyService.updateStrategyGroup(id, this.assetTaskGroupStrategyObject)
            .subscribe(out => {
              // console.log(out);
  ...
  filterAsset(): void {
    this._assetTaskGroupStrategyService.getAssetTaskGroupStrategyFilters(this.categoryid, this.category, this.taskGroupId, this.taskGroupDesc, this.taskTypeId, 
      this.frequencyId, this.tradeTypeId, this.operationalModeId, this.industryId, this.businessTypeId, this.assetClassTaxonomyId, 
      this.processFunctionId, this.assetSpecId, this.manufactureId, this.customerId, this.siteId)
      .subscribe(res => {
        this.default = false;
        this.loading = false;
        this.ELEMENT_DATA = res;
  ...
  goToATGEdit(id: number): void {
    if(this.canAccess('ReviewSiteTGS')) {
      this.router.navigate(["/main/site-task-group-edit", id]);
    }else{
      this.toastr.warning('You do not have permission to perform this action.', '');
    }
  ...
  downloadWorkIns(id: number, hierarchy: number, title: string): void{
    if(this.canAccess('GenerateSiteWorkInstruction')) {
      this.loading = true;
      this.isLoading = true;
      this.workInstTitle = title;
      this._assignTGSMaterialService.downloadHSM(id, hierarchy)
          .subscribe(res => {
              this.isLoading = false;
  ...
  openDialogTgs(val: any, data: any) {
    if(this.canAccess('ViewSiteTGSDetails')) {
      const dialogConfig = new MatDialogConfig();
      ...
      this._dataService.setData(dialogConfig.data);
      const dialogRef = this.dialog.open(SiteTgsViewModalComponent, dialogConfig);
      dialogRef.afterClosed().subscribe(res => {
          this.ngOnInit();
      });
    }else {
  ...
  deleteAction(id: number, name: string, tgsid: number): void {
    if(this.canAccess('DeleteSiteTGS')) {
      if (confirm("Are you sure to delete " + name + " ? This cannot be undone.")) {
        this._assetTaskGroupStrategyService.deleteAssetTaskGroupStrategy(id)
        .subscribe(res => {
            this._assetTaskGroupStrategyService.deleteTGSId(tgsid)
              .subscribe(out => {
